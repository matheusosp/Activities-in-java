...
public class GincanaAcampamento {
    public static void main(String[] args) {
        ...
        while(true){
          ...
          if(qtdpessoas==0) {
            ...
            String nomePessoa [] = new String[qtdpessoas];
            int numeroPessoa [] = new int[qtdpessoas];
            int index [] = new int[1];
            for (int i = 0; i < qtdpessoas; i++) {
                nomePessoa[i] = input.next();
                numeroPessoa[i] = Integer.parseInt(input.next());
            }
            int foco = numeroPessoa[0];
            if(foco % 2 != 0){
                index[0]=0;
            }else{
                index[0]=numeroPessoa.length;
            }
            for (int i = 0; i < qtdpessoas-1; i++) {
                if(foco % 2 != 0){
                    foco = verificaImpar(foco,numeroPessoa,index);
                }else{              
                    foco = verificaPar(foco,numeroPessoa, index);
                }
            }
            String ganhador = "";
            for (int i = 0; i < qtdpessoas; i++) {
                if(numeroPessoa[i]!=0){
                    ganhador = nomePessoa[i];
                }
            }
            System.out.println("Vencedor(a): "+ganhador);
          }  
    }
        }
    public static int verificaImpar(int foco,int numeroPessoa[],int index[]){
        int newfoco = 0;
        int contador = 0;
        int i = index[0];
        while(contador!=foco){
            i++;
            if(i == numeroPessoa.length){
                i=0;
            }
            if(numeroPessoa[i]==0){
                contador--;
            }
            newfoco = numeroPessoa[i];
            index[0] = i;
            contador++;
        }
        numeroPessoa[i] = 0;
    ...
    public static int verificaPar(int foco,int numeroPessoa[],int index[]){
        int newfoco = 0;
        int contador = 0;
        int i = index[0];
        while(contador!=foco){
